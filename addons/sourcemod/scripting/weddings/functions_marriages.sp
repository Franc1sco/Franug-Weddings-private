...
addMarriage(String:source_name[], String:source_id[], String:target_name[], String:target_id[]) {
	if(weddings_db == INVALID_HANDLE) {
		LogError("Unable to connect to database on addMarriage.");
	} else {
		new date;
...
checkMarriage(String:client_id[]) {
	...
	if(weddings_db == INVALID_HANDLE) {
		LogError("Unable to connect to database on checkMarriage.");
	} else {
...
revokeMarriage(String:client_id[]) {
	...
	if(weddings_db == INVALID_HANDLE) {
		LogError("Unable to connect to database on revokeMarriage.");
	} else {	
		decl String:client_id_temp[MAX_BUFFER_LENGTH];
...
findMarriages(String:client_id[]) {
	...
	if(weddings_db == INVALID_HANDLE) {
		LogError("Unable to connect to database on findMarriages.");
	} else {
		new Handle:data = CreateDataPack();		
...
updateMarriage(String:client_name[], String:client_id[], String:newName[], update_db, who) {
	...
	if(client_index == -1) {
		...
		if(GetClientName(client_index, newName, MAX_NAME_LENGTH) && !StrEqual(newName, client_name) && update_db) {		
			if(weddings_db == INVALID_HANDLE) {
				LogError("Unable to connect to database on updateMarriage.");
			} else {
 ...
 * @noreturn
 */
updateMarriageScore(String:client_id[]) {
	if(weddings_db == INVALID_HANDLE) {
		LogError("Unable to connect to database on updateMarriageScore.");
	} else {
		decl String:query[MAX_BUFFER_LENGTH];
		decl String:client_id_temp[MAX_BUFFER_LENGTH];
...
public Marriages_Find(Handle:owner, Handle:handle, const String:error[], any:data) {
	if(handle == INVALID_HANDLE) {
		...
		if(client != -1) {
			if(SQL_GetRowCount(handle) == 0) {
				PrintToChat(client, " [LOVE] %t", "no couples");
			} else {
				new score;		
				new date_temp;		
				...
				SetMenuTitle(couples_menu, "%t", "!couples menu title", show_couples);

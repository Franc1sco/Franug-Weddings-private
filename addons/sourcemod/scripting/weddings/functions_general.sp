...
checkClients() {
	...
	for(new i = 1; i <= MaxClients; i++) {
		...
		&& GetClientAuthId(i, AuthId_Engine, client_id, sizeof(client_id))) {
...
createTables() {
	if(weddings_db == INVALID_HANDLE) {
		LogError("Unable to connect to database on createTables.");
	} else {
...
resetTables(client) {
	if(weddings_db == INVALID_HANDLE) {
		LogError("Unable to connect to database on resetTables.");
	} else {
...
public DB_Create(Handle:owner, Handle:handle, const String:error[], any:data) {
	if(handle == INVALID_HANDLE) {
		...
		SQL_TQuery(weddings_db, DB_Create2, sql_createMarriages);
	}
}

public DB_Create2(Handle:owner, Handle:handle, const String:error[], any:data) {
	if(handle == INVALID_HANDLE) {
		LogError("Error creating tables in database. (%s)", error);
	} else {
		for(new i = 1; i <= MaxClients; i++) {
